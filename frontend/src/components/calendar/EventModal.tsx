...
const EventModal: React.FC<EventModalProps> = ({
  ...
  const extractTimeFromDateTime = useCallback(
    ...
    if (isOpen) {
      if (mode === "edit" && event) {
        ...
        const startDateForInput =
          event.startDateOnly || formatDateForInput(event.startDate);
        const endDateForInput =
          event.endDateOnly || formatDateForInput(event.endDate ?? null);
        ...
        setEventData({
          ...
          startTime:
            event.startTime ||
            extractTimeFromDateTime(event.startDate) ||
            "09:00",
          endTime:
            event.endTime ||
            extractTimeFromDateTime(event.endDate ?? null) ||
            "10:00",
        ...
        // Create mode - set default values to current date/time
        const now = new Date();
        ...
        const endTime = `${pad(endDateObj.getHours())}:${pad(
          endDateObj.getMinutes()
        )}`;

        // If end time spills into the next day → set endDate accordingly
  ...
  const validateField = (
    ...
    crossValidationData?: Partial<EventFormData>
  ) => {
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      const startTimeValid =
        eventData.isAllDay || validateField("startTime", eventData.startTime);
      const endTimeValid =
        eventData.isAllDay || validateField("endTime", eventData.endTime);
      ...
      if (
        ...
        console.warn(
          "⚠️ Event validation failed: Real-time validation errors found"
        );
      ...
      const validationData = {
        ...
        reminderMinutes: eventData.hasReminder
      ...
      if (mode === "create") {
        ...
        const response = await eventApiService.updateEvent(
          event.id,
          apiPayload
        );
      ...
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6">
          ...
          {showDeleteConfirm && mode === "edit" && (
            <div className="mb-6 p-6 bg-red-50 border border-red-200 rounded-xl shadow-sm">
              <div className="text-center mb-6">
                <div className="mx-auto w-12 h-12 bg-red-100 rounded-full flex items-center justify-center mb-4">
                  <AlertCircle className="w-6 h-6 text-red-600" />
                </div>
                <h4 className="text-lg font-semibold text-red-800 mb-2">Delete Event</h4>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    required
                  >
                    {Object.entries(EVENT_TYPE_OPTIONS).map(
                      ([type, config]) => (
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  >
                    {Object.entries(EVENT_PRIORITY_OPTIONS).map(
                      ([priority, config]) => (
              ...
              <div className="flex justify-between items-center mt-1">
                <div className="text-xs text-gray-500">
                  {eventData.childId === null
                    ? `Event will be created for all ${
                        children.filter((child) => child.id !== null).length
                      } children`
                    : `Event will be created for ${
                        children.find((child) => child.id === eventData.childId)
                          ?.name || "selected child"
                      } only`}
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      onBlur={(e) =>
                        handleFieldBlur("startTime", e.target.value)
                      }
            ...
            <div className="flex items-center space-x-4">
              <label className="flex items-center space-x-2">
                <input
                  ...
                  onChange={(e) =>
                    handleFieldChange("isAllDay", e.target.checked)
                  }
