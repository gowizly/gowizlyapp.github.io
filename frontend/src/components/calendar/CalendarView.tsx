...
const CalendarView: React.FC<CalendarViewProps> = ({
  ...
  const [showAllEventsModal, setShowAllEventsModal] = useState<{
    open: boolean;
    events: Event[];
  }>({ open: false, events: [] });
  ...
  const navigate = (amount: number) => {
    ...
    if (viewMode === "month")
      newDate.setUTCMonth(selectedDate.getUTCMonth() + amount);
    if (viewMode === "week")
      newDate.setUTCDate(selectedDate.getUTCDate() + amount * 7);
    if (viewMode === "day")
      newDate.setUTCDate(selectedDate.getUTCDate() + amount);
          ...
          >
            ...
            <div className="space-y-1 mt-2 overflow-visible">
              {dayEvents.slice(0, 3).map((event) => (
                <div
                  key={event.id}
                  className={`relative text-xs text-white px-2 py-1 rounded cursor-pointer ${
              ...
              {dayEvents.length > 3 && (
                <div
                  ...
                  onClick={() =>
                    setShowAllEventsModal({ open: true, events: dayEvents })
                  }
                >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-2xl w-full max-w-md">
            <div className="bg-purple-600 text-white p-4 flex justify-between items-center">
              ...
              <button
                onClick={() =>
                  setShowAllEventsModal({ open: false, events: [] })
                }
